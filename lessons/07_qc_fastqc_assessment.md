...
The HTML files represent the final reports generated by FastQC, let's take a closer look at them. In order to view them, we need to first transfer the file over to your laptop and then open it up in a browser. We will do this for `Mov10_oe_1.subset.fq` and we will use a software called [*Filezilla*](https://filezilla-project.org/) which you shoud have downloaded to your local computer prior to the workshop.

> ### What is FileZilla?
> FileZilla is a file transfer client with lots of useful features and an intuitive graphical user interface. It basically allows you to reliably move files securely between two computers using a point and click environment. It has cross-platform compatability so you can install it on any operating system.
...
You will see messages printed in teh message window in the top window pane, giving a you an indication of whether or not you have successfully connected to O2. Next, if this if your first time using Filezilla we recommend that you take some time to get familiar withe the basics of the interface. This [tutorial](https://wiki.filezilla-project.org/FileZilla_Client_Tutorial_(en)) is a helpful resource.


You will see two panels in the interface. On the left hand side you have the O2 filesystem, on the right hand side you have your laptop's filesystem. Both panels have a directory tree at the top and a detailed listing of the currently selected directory's contents at the bottom. In the right hand panel, navigate to where the HTML files are located on O2 `~/rnaseq/results/fastqc/`. Then decide where you would like to copy those files on your computer (Desktop is usually a good choice) and move to that directory on the left hand panel.

**Add a screenshot here?**

Find the HTML file for `Mov10_oe1`. **To copy it over**, you can double click it or drag it over to right hand side panel. Once you have the HTML file copied over to your laptop, find it on your Desktop or in a Finder window and open it up in a browser. 
...
In our plot above, we can see a drop in quality towards the ends of the reads, which could be explained by signal decay or phasing which are described in more detail below. No other worrisome signs are present, so the sequencing data from the facility is of good quality. 
