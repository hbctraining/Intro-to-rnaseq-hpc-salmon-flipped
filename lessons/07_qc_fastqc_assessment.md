...
The HTML files represent the final reports generated by FastQC, let's take a closer look at them. In order to view them, we need to first transfer the file over to your laptop and then open it up in a browser. We will do this for `Mov10_oe_1.subset.fq` and we will use [*FileZilla*](https://filezilla-project.org/).

> **NOTE:** You should have downloaded and installed FileZilla *prior to the workshop*!.
...
Now that you are connected you will see two panels in the interface. On the left hand side you have the O2 filesystem. Navigate to where the HTML files are located `~/rnaseq/results/fastqc/`. You can do this by typing the path into the search bar at the top of the panel. Find the HTML file for `Mov10_oe1` and double click or drag it over to right hand side panel (which is your laptop filesystem).

**Add a screenshot here?**

Once you have the HTML file copied over to your laptop, you should be able to open it in a browser. 
